...
class FormGroup
{
    ...
    public function checkbox(string $name, string $label, $checked = false, $value = '1'): Div
    {
        return $this->wrapper()->children([
            $this->html->label()
                       ->for($name)
                       ->html($this->html->checkbox($name, $checked, $value)
                                         ->class('form-control').' '.__($label).($this->required ? '*' : ''))
                       ->class('-checkbox'),
        ]);
    }
